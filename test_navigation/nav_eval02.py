...
Pass criteria (used only for the console summary; no longer logged per-trial):
  e_trans  <= 0.05 m   (±5 cm)
  |e_head| <= 5.0 deg  (±5°)

The Nav2 action namespace is auto-detected from the ROS graph by default.
Set `robot_namespace` in the config to pin it (use '' for the global
namespace, or 'auto' to force discovery).
...
# Suffix the rcl action layer publishes for every NavigateToPose server.
_NAV_ACTION_BASENAME = 'navigate_to_pose'
_NAV_STATUS_SUFFIX = f'/{_NAV_ACTION_BASENAME}/_action/status'
...
class NavEvaluator(Node):
    def __init__(self, cfg: dict):
        ...
        # Resolve the robot namespace first; frames/topics default off it.
        self.robot_ns = self._resolve_namespace()           # '' or 'apple' (no slashes)
        ns_prefix = f'{self.robot_ns}/' if self.robot_ns else ''
        action_name = f'{ns_prefix}{_NAV_ACTION_BASENAME}'
        self.get_logger().info(
            f'Robot namespace: "{self.robot_ns or "/"}"  |  Nav2 action: {action_name}'
        )

        self.map_frame = cfg.get('map_frame') or 'map'
        self.base_frame = cfg.get('base_frame') or f'{ns_prefix}base_link'
        self.pose_source = cfg.get('pose_source', 'tf')        # 'tf' or 'topic'
        self.pose_topic = cfg.get('pose_topic') or f'/{ns_prefix}amcl_pose'
    ...
    # ----- namespace / action discovery -----

    def _resolve_namespace(self) -> str:
        """Return the robot namespace (no leading/trailing slashes; '' = global).

        Honours `robot_namespace` (or `namespace`) in the config when given;
        otherwise scans the ROS graph for a running NavigateToPose server and
        infers the namespace from its action name.
        """
        ns = self.cfg.get('robot_namespace', self.cfg.get('namespace', 'auto'))
        if ns is not None and str(ns).lower() != 'auto':
            return str(ns).strip('/')

        discovered = self._discover_nav_action(timeout_sec=10.0)
        if discovered:
            ns = discovered.rsplit(f'/{_NAV_ACTION_BASENAME}', 1)[0].strip('/')
            self.get_logger().info(f'Auto-detected robot namespace: "{ns or "/"}"')
            return ns

        self.get_logger().warn(
            'Could not auto-detect a NavigateToPose server; assuming the global '
            'namespace. Set robot_namespace in the config to override.'
        )
        return ''

    def _discover_nav_action(self, timeout_sec: float = 10.0) -> str | None:
        """Return the first `<ns>/navigate_to_pose` action found on the graph."""
        deadline = time.time() + timeout_sec
        while rclpy.ok() and time.time() < deadline:
            candidates = sorted(
                name[: -len('/_action/status')]
                for name, _types in self.get_topic_names_and_types()
                if name.endswith(_NAV_STATUS_SUFFIX)
            )
            if candidates:
                # Prefer a namespaced server over a bare global one if both exist.
                for name in candidates:
                    if name != f'/{_NAV_ACTION_BASENAME}':
                        return name
                return candidates[0]
...
def write_xlsx(path: Path, rows: list, goal_ids: list) -> None:
    ...
    s.cell(row=5, column=2, value=f'=IFERROR(AVERAGE({jR}), 0)')
    s.cell(row=5, column=2).number_format = '0.0000'
    s.cell(row=5, column=3, value=f'=IFERROR(MAX({jR}), 0)')
    s.cell(row=5, column=3).number_format = '0.0000'
    s.cell(row=5, column=4, value=f'=IFERROR(SUMPRODUCT(ABS({kR}))/A5, 0)')
    s.cell(row=5, column=4).number_format = '0.000'
    s.cell(row=5, column=5, value=f'=IFERROR(SUMPRODUCT(MAX(ABS({kR}))), 0)')
    s.cell(row=5, column=5).number_format = '0.000'
    ...
    for gid in goal_ids:
        ...
        s.cell(row=row_idx, column=3, value=f'=IFERROR(AVERAGEIF({cR}, "{gid}", {jR}), 0)')
        s.cell(row=row_idx, column=3).number_format = '0.0000'
        s.cell(row=row_idx, column=4,
               value=f'=IFERROR(SUMPRODUCT(MAX(({cR}="{gid}")*{jR})), 0)')
        s.cell(row=row_idx, column=4).number_format = '0.0000'
        s.cell(row=row_idx, column=5,
               value=f'=IFERROR(SUMPRODUCT(({cR}="{gid}")*ABS({kR}))/B{row_idx}, 0)')
        s.cell(row=row_idx, column=5).number_format = '0.000'
        s.cell(row=row_idx, column=6,
               value=f'=IFERROR(SUMPRODUCT(MAX(({cR}="{gid}")*ABS({kR}))), 0)')
        s.cell(row=row_idx, column=6).number_format = '0.000'
        for col in range(1, 7):
            s.cell(row=row_idx, column=col).font = body_font
        row_idx += 1

    for col, w in [('A', 14), ('B', 10),
                   ('C', 18), ('D', 18), ('E', 22), ('F', 22)]:
...
def main():
        ...
        for g in goals:
            ...
            for k in range(repeats):
                ...
                if actual is None:
                    ...
                    xa, ya, ta = actual
                    # Translation error: straight-line distance between the
                    # commanded (xg, yg) and the measured (xa, ya) position.
                    e_t = math.hypot(xa - xg, ya - yg)
                    # Heading error magnitude: wrap (actual - goal) into
                    # (-pi, pi] so 359deg counts as 1deg, then take |.| — we only
                    # care how far off we are, not which way.
                    e_h_deg = abs(math.degrees(wrap_angle(ta - tg)))
                    # A trial "passes" only if Nav2 reported success AND both
                    # errors are within the configured thresholds.
                    ok = (
                        status == 'SUCCEEDED'
                        and e_t <= e_trans_thresh
                        and e_h_deg <= e_head_thresh
                    )
                    if ok:
                        passed += 1
                    note = '' if status == 'SUCCEEDED' else status
                    row = [
                        ts, trial_id, gid,
                        round(xg, 4), round(yg, 4), round(tg_deg, 3),
                ...
                node.get_logger().info(
                    f'  result: e_trans={row[9]} m  e_head={row[10]} deg  status={status}'
                )
    ...
    print(f'Passed     : {passed}  (e_trans <= {e_trans_thresh} m, '
          f'|e_head| <= {e_head_thresh} deg)')
