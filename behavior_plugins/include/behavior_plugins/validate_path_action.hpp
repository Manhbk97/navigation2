...
#ifndef BEHAVIOR_PLUGINS__VALIDATE_PATH_ACTION_HPP_
#define BEHAVIOR_PLUGINS__VALIDATE_PATH_ACTION_HPP_
...
/**
 * @brief A nav2_behavior_tree::BtServiceNode class that validates a path by
 * calling the IsPathValid service on the path provided in the input port.
 *
 ...
 * @endcode
 */
class ValidatePath : public nav2_behavior_tree::BtServiceNode<nav2_msgs::srv::IsPathValid>
{
public:
  /**
    ...
    return providedBasicPorts(
      {
        ...
        BT::InputPort<uint16_t>("max_cost", 254, "Maximum cost of the path (0-255)"),
        BT::InputPort<bool>(
          "consider_unknown_as_obstacle", false,
        ...
        BT::InputPort<std::string>(
          "footprint", "",
          "Custom footprint as bracketed array of arrays, e.g. "
...
private:
  // Service field `max_cost` is uint8 — using uint16_t for BT's port type
  // because BT.CPP's string-to-uint8 conversion parses as char, which is wrong.
  // The narrowing to uint8_t happens when filling the request.
  uint16_t max_cost_;
...
#endif  // BEHAVIOR_PLUGINS__VALIDATE_PATH_ACTION_HPP_
