...
def _control_world(agent_self, paused):
    req = ControlWorld.Request()
    ctl = WorldControl()
    ctl.pause = paused
    req.world_control = ctl
    while not agent_self.gazebo_control.wait_for_service(timeout_sec=1.0):
        agent_self.get_logger().info('gazebo control service not available, waiting again...')
    future = agent_self.gazebo_control.call_async(req)
    while rclpy.ok():
        rclpy.spin_once(agent_self)
        if future.done():
            return
...
def pause_simulation(agent_self, real_robot):
    if real_robot:
        return
    _control_world(agent_self, True)

def unpause_simulation(agent_self, real_robot):
    if real_robot:
        return
    _control_world(agent_self, False)
...
def get_scan_count():
    ...
    tree = ET.parse('/home/rgt/ros2_robot_ws/src/turtlebot3_deepRL' + '/src/turtlebot3_simulations/turtlebot3_gazebo/models/turtlebot3_burger/model.sdf')
